...
if ( ! function_exists( 'trav_get_user_currency' ) ) {
	function trav_get_user_currency() {
		...
		if ( empty( $trav_options['site_currencies'] ) ) $trav_options['site_currencies'] = array();
...
if ( ! function_exists( 'trav_init_currency' ) ) {
	function trav_init_currency() {
		...
		//exchange_rate init
		if ( empty( $_SESSION['exchange_rate'] ) ) $_SESSION['exchange_rate'] = trav_currency_converter( 1 , $def_currency, $_SESSION['user_currency'] );
		//currency_symbol init
		if ( empty( $_SESSION['currency_symbol'] ) ) $_SESSION['currency_symbol'] = trav_get_currency_symbol( $_SESSION['user_currency'] );
	}
}
