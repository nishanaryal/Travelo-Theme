...
if ( have_posts() ) {
	while ( have_posts() ) : the_post();
		...
		$tour_price = get_post_meta( $tour_id, 'trav_tour_min_price', true );

		$tour_itinerary = get_post_meta( $tour_id, 'trav_tour_itinerary', true );
		$tour_inc_exc = get_post_meta( $tour_id, 'trav_tour_inc_exc', true );
		$tour_equipments = get_post_meta( $tour_id, 'trav_tour_equipments', true );
		$tour_faqs = get_post_meta( $tour_id, 'trav_tour_faqs', true );
		$tour_notes = get_post_meta( $tour_id, 'trav_tour_notes', true );
		$tour_route_map = get_post_meta( $tour_id, 'trav_tour_route_map', true );

		$total_days = get_post_meta( $tour_id, 'trav_tour_total_days', true );
					...
					<div id="main" class="col-sm-8 col-md-9">
						<div <?php post_class(); ?>>
							<div class="image-box">
								...
								<?php if ( ! empty( $tour_price ) && ! empty( $tour_price ) ) : ?>
									<div class="trip-cost">
										<div class="price-from">
											<h5>All inclusive price</h5>
											<span class="currency-sign">RUB</span> <span class="prd-cost"><?php echo esc_html( $tour_price ) ?></span> <span class="currency">/person</span>               
										</div><!--price-from-->
									</div>
								<?php endif; ?>
								...
								<?php if ( empty( $sc_list_pos ) ) : ?>
									...
									<div class="entry-content">
										...
										<div class="vc_row wpb_row row">
											<div class="col-sm-12 ">
												<div class="tab-container clearfix style3">
													<ul class="tabs clearfix">
														<?php if ( ! empty( $tour_itinerary ) && ! empty( $tour_itinerary ) ) : ?>
															<li class="active"><a href="#itinerary" data-toggle="tab">Trip Itinerary</a></li>
														<?php endif; ?>
														<?php if ( ! empty( $tour_equipments ) && ! empty( $tour_equipments ) ) : ?>
														<li class=""><a href="#1459444920293-9" data-toggle="tab">Includes/Excludes</a></li>
														<?php endif; ?>

														<?php if ( ! empty( $tour_equipments ) && ! empty( $tour_equipments ) ) : ?>
															<li class=""><a href="#1459444920351-10" data-toggle="tab">Trekking Equipments</a></li>
														<?php endif; ?>

														<?php if ( ! empty( $tour_notes ) && ! empty( $tour_notes ) ) : ?>
															<li class=""><a href="#1459444920511-5" data-toggle="tab">Notes</a></li>
														<?php endif; ?>

														<?php if ( ! empty( $tour_route_map ) && ! empty( $tour_route_map ) ) : ?>
															<li class=""><a href="#1459444920293-12" data-toggle="tab">Route Map</a></li>
														<?php endif; ?>

														<?php if ( ! empty( $tour_faqs ) && ! empty( $tour_faqs ) ) : ?>
															<li class=""><a href="#1459444920216-8" data-toggle="tab">FAQs</a></li>
															<?php endif; ?>
													</ul>
													<div class="tab-content">
														<div id="1459444920216-7" class="tab-pane fade active in">
														</div>
														<div id="1459444920293-9" class="tab-pane fade">

														</div>
														<div id="1459444920351-10" class="tab-pane fade">
														</div>
														<div id="1459444920511-5" class="tab-pane fade">


														</div>
														<div id="1459444920293-12" class="tab-pane fade">

														</div>
														<div id="1459444920216-8" class="tab-pane fade">
														</div>
													</div>
												</div>
											</div> 
